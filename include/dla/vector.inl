...
#undef MAKE_VECTOR_SCALAR_BINARY_ASSIGNEMT_OPERATOR
    ...
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::reverse_iterator vec<T, N>::rbegin() noexcept { return { *this, N - 1 }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_reverse_iterator vec<T, N>::rbegin() const noexcept { return { *this, N - 1 }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::reverse_iterator vec<T, N>::rend() noexcept { return { *this, -1 }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_reverse_iterator vec<T, N>::rend() const noexcept { return { *this, -1 }; }

    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_iterator vec<T, N>::cbegin() noexcept { return { *this, 0 }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_iterator vec<T, N>::cend() const noexcept { return { *this, N }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_reverse_iterator vec<T, N>::crbegin() noexcept { return { *this, N - 1 }; }
    template<class T, std::size_t N>
    constexpr typename vec<T, N>::const_reverse_iterator vec<T, N>::crend() noexcept { return { *this, -1 }; }
}