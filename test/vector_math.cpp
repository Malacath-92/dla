...
DLA_TEST_CONSTEXPR auto abs_vec = dla::abs(vector);
...
DLA_TEST_CONSTEXPR auto min_vec = dla::ivec4(dla::min(vector, other_vector));
DLA_TEST_CONSTEXPR auto max_vec = dla::ivec4(dla::max(vector, other_vector));
...
TEST_CASE("Test math operations on vectors", "[vector_math]") {
	...
	REQUIRE(min_vec == dla::ivec4{ 0, 1, -1, -1 });
