...
int main() {
	...
	constexpr auto cbrt_three = math::cbrt(three);
	constexpr auto three_again = cbrt_three * cbrt_three * cbrt_three;
	static_assert(math::abs(three - three_again) < 0.000001f);

	constexpr auto length_sqrt = math::sqrt(length);
	constexpr auto length_cbrt = math::cbrt(length);
	auto length_full = math::pow<2>(length_sqrt);
	length_full = math::pow<3>(length_cbrt);
	constexpr auto force_sqrt = math::sqrt(force);
	constexpr auto force_cbrt = math::cbrt(force);
